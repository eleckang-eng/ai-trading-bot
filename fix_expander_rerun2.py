...
'''
    ...
    # 2. Add st.rerun() properly
    # Button 1: 균등 그리드 생성
    btn1_target = '        if st.button("📊 균등 그리드 생성", use_container_width=True):\n            bp = _calc_base_price()\n            sell_list, buy_list = _generate_grid(bp, order_quantity)\n            st.session_state.grid_sell_list = sell_list\n            st.session_state.grid_buy_list = buy_list\n            st.session_state.show_grid_preview = True\n            st.session_state.confirm_batch_order = False'
    if btn1_target in c:
        c = c.replace(btn1_target, btn1_target + '\n            st.rerun()')
        
    # Button 2: 계단식 그리드 생성
    btn2_target = '        if st.button("📈 계단식 그리드 생성", use_container_width=True):\n            bp = _calc_base_price()\n            sell_list, buy_list = _generate_stair_grid(bp, order_quantity, stair_steps, stair_add_qty, stair_direction)\n            st.session_state.grid_sell_list = sell_list\n            st.session_state.grid_buy_list = buy_list\n            st.session_state.show_grid_preview = True\n            st.session_state.confirm_batch_order = False'
    # Wait, the second button was replaced. But the function `_generate_stair_grid` didn't exist in backup!
    # Ah! In my `dashboard_backup2.py`, there is NO `_generate_stair_grid` function!
    # Let's find exactly what the 계단식 button looks like.
