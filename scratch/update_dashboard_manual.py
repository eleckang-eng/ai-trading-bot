...
file_path = r'app/dashboard.py'
with codecs.open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

old_manual = '''        c_buy, c_sell = st.columns(2)
        with c_buy:
...
new_manual = '''        c_buy, c_sell = st.columns(2)
        with c_buy:
            if st.button("🔴 매수", width="stretch"):
                if m_price <= 0:
                    st.error("주문 가격을 1원 이상 입력하십시오.")
...
with codecs.open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)
print('Manual order confirmation added successfully!')
