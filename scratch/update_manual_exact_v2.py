...
if start_idx != -1:
    ...
    new_code = '''        c_buy, c_sell = st.columns(2)
        with c_buy:
            if st.button("🔴 매수", width="stretch", key="btn_manual_buy"):
                if m_price <= 0:
                    ...
                    st.rerun()
        with c_sell:
            if st.button("🔵 매도", width="stretch", key="btn_manual_sell"):
                if m_price <= 0:
                    st.error("주문 가격을 1원 이상 입력하십시오.")
                else:
    ...
    with m_c4:
        st.write("")
        st.write("")
        if st.button("🗑️ 미체결 일괄 취소", width="stretch", type="primary", key="btn_manual_cancel_all"):
    ...
    if st.session_state.get("confirm_manual_order"):
        ...
        with cc1:
            if st.button("✅ 주문 전송", type="primary", use_container_width=True, key="btn_confirm_manual"):
                try:
                    import requests
                    res = requests.post(f"{API_URL}/order/manual", json=order, timeout=5)
                    data = res.json()
                    if data.get("status") == "success":
                        msg_placeholder.success(data.get("message", f"{side_kor} 성공"))
                    else:
                        msg_placeholder.error(data.get("message", f"{side_kor} 에러"))
                except Exception as e:
                    msg_placeholder.error(f"요청 실패: {e}")
                
                st.session_state["confirm_manual_order"] = None
                import time; time.sleep(1.5); st.rerun()
        with cc2:
            if st.button("❌ 취소", use_container_width=True, key="btn_cancel_manual"):
                st.session_state["confirm_manual_order"] = None
                st.rerun()
    ...
    if st.session_state.get("confirm_cancel_all"):
        ...
        cc1, cc2 = st.columns(2)
        with cc1:
            if st.button("✅ 전체 취소 실행", type="primary", use_container_width=True, key="btn_confirm_cancel_all"):
                try:
                    import requests
                    res = requests.post(f"{API_URL}/order/cancel_all", timeout=5)
                    msg_placeholder.success(res.json().get("message", "모든 주문이 취소되었습니다."))
                except:
                    msg_placeholder.error("취소 요청 실패")
                
                st.session_state["confirm_cancel_all"] = False
                import time; time.sleep(1.5); st.rerun()
        with cc2:
            if st.button("❌ 돌아가기", use_container_width=True, key="btn_cancel_cancel_all"):
                st.session_state["confirm_cancel_all"] = False
                st.rerun()
'''
