...
if old_manual_warn in content:
    content = content.replace(old_manual_warn, new_manual_warn)
else:
    print('Could not find old_manual_warn')


# 2. Batch order overlap check
...
if old_batch_warn in content:
    content = content.replace(old_batch_warn, new_batch_warn)
else:
    print('Could not find old_batch_warn')

with codecs.open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)
print('Overlap check update finished.')
